...
const MembershipCard = ({
  title, subtitle, features, isPopular = false,
  ctaText = "TRY FOR FREE", learnMore = true, delay = 0, bgImage = ""
}) => {
    ...
    >
      {bgImage && (
        <div className="absolute inset-0 rounded-2xl overflow-hidden">
          <div className="w-full h-full bg-cover bg-center transform group-hover:scale-110 transition-transform duration-700" style={{ backgroundImage: `url(${bgImage})` }} />
          <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/50 to-black/30" />
        </div>
      )}
      <div className={`relative p-6 lg:p-8 rounded-2xl border backdrop-blur-sm transition-all duration-500 ${
        isPopular 
        ...
        <div className="space-y-3">
          ...
          {learnMore && (
            <button className="w-full py-3 px-6 rounded-lg font-semibold bg-transparent border border-white/30 text-white hover:bg-white/10 transition-all duration-300">
              LEARN MORE
            </button>
          )}
        </div>
